...
void do_prune(
  ...
  decision_tree_model<T> model, model2;

  if (binary_mode) {
    timer.reset();
    model.loadbinary(input);
    timer.show("load model: ");
    model2 = model.prune(min_info_gain);
    timer.show("prune tree: ");
    model2.savebinary(output);
  ...
  RLOG(TRACE) << "<BEFORE PRUNING>" << std::endl << model << std::endl;
  RLOG(TRACE) << "<AFTER PRUNING>" << std::endl << model2;
}
...
po::variables_map parse(int argc, char** argv) {
  ...
  posarg_settings.add("input", 1).add("output", 1);
  ...
  optarg_desc.add_options()(
    ...
    "infogain,g", po::value<double>()->default_value(0.0),
