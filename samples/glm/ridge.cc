...
do_train_impl(MATRIX& mat, dvector<T>& lbl, 
  ...
  if (solver == "sgd") {
    lm = ridge_regression_with_sgd::train(std::move(mat), lbl, num_iteration, 
                                          alpha, minibatch_fraction, regParam,
                                          intercept, convTol, mType);
  }
  else if (solver == "lbfgs") {
    lm = ridge_regression_with_lbfgs::train(std::move(mat), lbl, num_iteration, 
                                            alpha, hist_size, regParam,
                                            intercept, convTol, mType);
  }
...
int main(int argc, char* argv[]) {
  ...
  if (dtype == "float") {
    if(ispredict) do_predict<float> (input, model, output, binary, isdense);
                         ...
                         regParam, intercept, convTol, mType, binary, isdense);
  }
