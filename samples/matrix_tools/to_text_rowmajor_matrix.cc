...
template <class T>
void to_text_rowmajor_matrix(string& input, string& output) {
  auto m = make_rowmajor_matrix_loadbinary<T>(input);
  if (output == "-") cout << m;
  else m.save(output);
}
...
int main(int argc, char* argv[]) {
  ...
  if(argmap.count("double")) {
    to_text_rowmajor_matrix<double>(input, output);
  } else if(argmap.count("float")) {
    to_text_rowmajor_matrix<float>(input, output);
  } else if(argmap.count("int")) {
    to_text_rowmajor_matrix<int>(input, output);
  } else {
