...
int main(int argc, char* argv[]){
    ...
    opt.add_options()
        ...
        ("method,m" , value<std::string>(), "method (sssp_BF_spMV, sssp_BF_2DMerge, sssp_BF_idxSort)")
...
//        ("fout" , " enable file output for the search result" )            
        ("upperbound,u" ,value<size_t>(), " upperbound for 2DMerge (default: 256)")
        ...
        ("enquiry,e" , "enable enquiry for sssp" );     
...
//       if_scale_free = true;
//    }     
    if(argmap.count("enquiry")){
       if_enq_sssp = true;
    } 
    if(argmap.count("upperbound")){
       upperbound = argmap["upperbound"].as<size_t>();;
    } 
...
//    CCptr->set_if_fout(if_fout);
    ...
    //sssp
    if(method_name == "sssp_BF_spMV"){
        CCptr->sssp_bf_spmv(if_enq_sssp , source_vertex);
    }
    else if(method_name == "sssp_BF_2DMerge"){
        CCptr->sssp_bf_spmspv_2dmerge(if_enq_sssp, source_vertex, upperbound);
    }   
    else if(method_name == "sssp_BF_idxSort"){
        CCptr->sssp_bf_spmspv_sort(if_enq_sssp, source_vertex);
    }    
    else{
