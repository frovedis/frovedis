...
void do_train(
  ...
  auto builder = make_gradient_boosted_trees_builder(strategy);
...
void do_train(const po::variables_map& argmap) {
  do_train<T>(
    ...
    tree::get_loss_type(argmap["loss"].as<std::string>()),
    argmap["learning-rate"].as<double>(),
    argmap.count("binary")
  );
}
...
po::variables_map parse(int argc, char** argv) {
  ...
  optarg_desc.add_options()
    ...
    ("loss", po::value<std::string>()->default_value("default"),
     "a loss function")
    ("learning-rate,s", po::value<double>()->default_value(0.1),
     "a learning rate (shrinkage)")
