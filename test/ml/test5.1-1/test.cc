...
BOOST_AUTO_TEST_CASE( frovedis_test )
{
    ...
    auto b_mat = make_crs_matrix_load<double>(b_matf);
    auto b_lbl = make_dvector_loadline<double>(b_lblf);
    auto m1 = bernoulli_nb<double,crs_matrix<double>,crs_matrix_local<double>>(b_mat,b_lbl);
    auto b_tmat = make_crs_matrix_local_load<double>(b_matf);
    auto pd1 = m1.predict(b_tmat);
    //for(auto& e: pd1) std::cout << e << " "; std::cout << std::endl;

    auto m_mat = make_crs_matrix_load<double>(m_matf);
    auto m_lbl = make_dvector_loadline<double>(m_lblf);
    auto m2 = multinomial_nb<double,crs_matrix<double>,crs_matrix_local<double>>(m_mat,m_lbl);
    auto m_tmat = make_crs_matrix_local_load<double>(m_matf);
