...
all:
	...
	cd src/frovedis/text; make -f Makefile.${TARGET} -j ${JOBS}
...
install:
	...
	cd src/frovedis/text; make -f Makefile.${TARGET} install
...
clean:
	...
	cd src/frovedis/text; make -f Makefile.${TARGET} clean
ifeq (${BUILD_FOREIGN_IF},true)
