...
class FrovedisDataframe :
    ...
    def __init__(self, df=None):
      ...
      self.__cols  = None
      self.__types = None
      if df is not None: self.load(df)
    ...
    def release(self): 
      if self.__fdata is not None:
         ...
         excpt = rpclib.check_server_exception()
         if excpt["status"]: raise RuntimeError(excpt["info"]) 
         for cname in self.__cols: del self.__dict__[cname]
         self.__fdata = None 
         self.__cols  = None
         self.__types = None
    ...
    def load(self, df):
      ...
      (host,  port) = FrovedisServer.getServerInstance()
      self.__fdata = rpclib.create_frovedis_dataframe(host, port,tptr,col_names,dptr,size)
      excpt = rpclib.check_server_exception()
      if excpt["status"]: raise RuntimeError(excpt["info"]) 
    ...
    def filter_frovedis_dataframe(self, opt):
      ...
      for item in ret.__cols: ret.__dict__[item] = self.__dict__[item]
      (host,  port) = FrovedisServer.getServerInstance()
      ret.__fdata = rpclib.filter_frovedis_dataframe(host, port,self.get(),opt.get())
      excpt = rpclib.check_server_exception()
      if excpt["status"]: raise RuntimeError(excpt["info"]) 
      return ret
    ...
    def select_frovedis_dataframe(self, targets):
      ...
      (host,  port) = FrovedisServer.getServerInstance()
      ret.__fdata = rpclib.select_frovedis_dataframe(host, port,self.get(),ptr_arr,sz)
      excpt = rpclib.check_server_exception()
      if excpt["status"]: raise RuntimeError(excpt["info"]) 
      return ret
    ...
    def groupby(self,  by=None, axis=0, level=None, 
      ...
      (host,  port) = FrovedisServer.getServerInstance()
      fdata = rpclib.group_frovedis_dataframe(host, port,self.get(),ptr_arr,sz)
      excpt = rpclib.check_server_exception()
      if excpt["status"]: raise RuntimeError(excpt["info"]) 
      return grouped_df.FrovedisGroupedDataframe().load_dummy(fdata, g_by,types,
                                                   self.__cols, self.__types)
    ...
    def merge(self,  right, on=None, how='inner', left_on=None, right_on=None, 
      ...
      if(on): #if key name is same in both dataframes
        ...
        dfopt = self.__evaluate(df, on,on) # currently exception at frovedis server
      ...
      for item in self.__cols: ret.__dict__[item] = self.__dict__[item]
      for item in df.__cols: 
        if item not in ret.__dict__: ret.__dict__[item] = df.__dict__[item]

      (host,  port) = FrovedisServer.getServerInstance()
      ret.__fdata = rpclib.merge_frovedis_dataframe(host, port,self.get(),df.get(),
    ...
    def join(self,  right, on, how='inner', 
      ...
      return self.merge(right, on=on,how=how,suffixes=suffix,sort=sort,join_type=join_type) 
    ...
    def get(self): return self.__fdata
    ...
    def count(self,  columns=None):
      if self.__fdata is None: 
        raise ValueError("Operation on invalid frovedis dataframe!")
      if not columns: return self.__get_stat('count', self.__cols) # count of all columns
      vtype = type(columns).__name__
      if(vtype == 'str'): by = [columns]
      elif(vtype == 'list'): by = columns 
    ...
    def __get_column_types(self,  columns):
      ...
      for i in range(0,  sz):
        item = columns[i]
        if item not in self.__cols: raise ValueError("No column named: ",  item)
        else:
    ...
    def __get_stat(self,  name, columns, types=None):
      ...
      if (name == 'min'): 
        ...
        ret = rpclib.get_min_frovedis_dataframe(host, port,self.get(),cols_ptr,tptr,sz)
      elif (name == 'max'): 
        if not types: raise ValueError("type of target columns is missing for max calculation")
        ret = rpclib.get_max_frovedis_dataframe(host, port,self.get(),cols_ptr,tptr,sz)
      elif (name == 'sum'): 
        if not types: raise ValueError("type of target columns is missing for sum calculation")
        ret = rpclib.get_sum_frovedis_dataframe(host, port,self.get(),cols_ptr,tptr,sz)
      elif (name == 'std'): 
        if not types: raise ValueError("type of target columns is missing for std calculation")
        ret = rpclib.get_std_frovedis_dataframe(host, port,self.get(),cols_ptr,tptr,sz)
      elif (name == 'avg'): 
        ret = rpclib.get_avg_frovedis_dataframe(host, port,self.get(),cols_ptr,sz)
      elif (name == 'count'): 
        ret = rpclib.get_cnt_frovedis_dataframe(host, port,self.get(),cols_ptr,sz)
    ...
    def __get_numeric_columns(self):
      ...
      for i in range(0,  len(self.__cols)):
        if (self.__types[i] != DTYPE.STRING): cols.append(self.__cols[i])
      return cols
    ...
    def to_frovedis_rowmajor_matrix(self, t_cols,dtype=np.float32): #default type: float
      if self.__fdata is None: 
        raise ValueError("Operation on invalid frovedis dataframe!")
    ...
    def to_frovedis_colmajor_matrix(self, t_cols,dtype=np.float32): #default type: float
      if self.__fdata is None: 
        raise ValueError("Operation on invalid frovedis dataframe!")
      for item in t_cols: # implicit checks for iterable on 't_cols'
        if item not in self.__cols: raise ValueError("No column named: ",  item)
    ...
    def to_frovedis_crs_matrix_using_info(self, info,dtype=np.float32): #default type: float 
      if self.__fdata is None: 
        raise ValueError("Operation on invalid frovedis dataframe!")
      if info.get() is None: 
      ...
      (host,  port) = FrovedisServer.getServerInstance()
      if (dtype == np.float32):
        dmat = rpclib.df_to_crs_using_info(host, port,self.get(),info.get(),DTYPE.FLOAT)
      elif (dtype == np.float64):
        dmat = rpclib.df_to_crs_using_info(host, port,self.get(),info.get(),DTYPE.DOUBLE)
      else: 
        raise TypeError("Supported types: float32/float64; Found: " + dtype.__name__)
      excpt = rpclib.check_server_exception()
      if excpt["status"]: raise RuntimeError(excpt["info"])
