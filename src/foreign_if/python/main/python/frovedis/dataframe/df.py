...
class DataFrame(SeriesHelper):
    ...
    def __clip_axis1_helper(self, lower=None, upper=None, axis=None, inplace=False):
        ...
        lb_types = np.array([TypeUtil.to_id_dtype( \
                           np.dtype(get_python_scalar_type(i))) for i in lower], \
                           dtype=c_short)
        import datetime
        for i in range(len(lb_types)):
            if lb_types[i] == DTYPE.DATETIME:
                lb_str[i] = str(lower[i].value)
        ub_str = [str(i) for i in upper]
        ub_types = np.array([TypeUtil.to_id_dtype( \
                           np.dtype(get_python_scalar_type(i))) for i in upper], \
                           dtype=c_short)
