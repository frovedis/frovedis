package test.regression.LinearRegression;
...
object LNRFrovedisDemo {
  def main(args: Array[String]): Unit = {
    ...
    // -------- configurations --------
    val conf = new SparkConf().setAppName("LNRFrovedis") 
    ...
    if (opt == "sgd") {
      val m1 = LinearRegressionWithSGD.train(training,numiter,stepsize,mb)
      ...
      val m1 = LinearRegressionWithLBFGS.train(training,numiter,stepsize,hs) 
      //val tvec = test.map(_.features)
      //m1.predict(tvec).collect.foreach(println)
      //test.map(_.label).collect.foreach(println)
      val valuesAndPreds = test.map { point =>
