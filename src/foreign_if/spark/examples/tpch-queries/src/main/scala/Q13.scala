...
import com.nec.frovedis.sql.{FrovedisDataFrame, DFMemoryManager}
...
*/
    ...
    val forder = new FrovedisDataFrame(order, "o_custkey", "o_comment")
    t.show("data transfer: ")

    val Cspecial = $$"o_comment".like("%special%requests%")

    val fret = fcustomer.join(forder, $$"c_custkey" === forder("o_custkey") && !Cspecial, "left_outer")
      .groupBy($$"o_custkey")
      .agg(count($$"o_orderkey").as("c_count"))
      .groupBy($$"c_count")
      .agg(count($$"o_custkey").as("custdist"))
