...
object FrovedisDataframeDemo {
  def main(args: Array[String]): Unit = {
    ...
    // filter demo
    df1.filter($$"Age" > 25).show(); println
    df1.filter($$"Age" < 25).show(); println
    df1.filter($$"Age" === 19).show(); println
    df1.filter($$"Age" !== 19).show(); println
    df1.filter($$"Age" >= 19).show(); println
    df1.filter($$"Age" <= 19).show(); println
    df1.filter($$"Country" === "Japan").show(); println
    df1.filter($$"Country" !== "USA").show(); println
    df1.filter(($$"Country" !== "Japan") && ($$"Age" > 19)).show(); println
