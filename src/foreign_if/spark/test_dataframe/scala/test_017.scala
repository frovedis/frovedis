...
object FrovedisDataframeDemo {
  def main(args: Array[String]): Unit = {
    ...
    val peopleDF = sc.textFile("./input/new.txt")
                     .map(_.split(","))
                     .map(attributes => (attributes(0).trim.toInt, 
                                         attributes(1).trim, attributes(2).trim.toInt))
    ...
    // sort demo
    df1.sort($$"Subject").show(); println
    df1.sort("Id").show(); println

    df1.sort("Id","Subject").show(); println
    df1.sort($$"Id",$$"Subject").show(); println
