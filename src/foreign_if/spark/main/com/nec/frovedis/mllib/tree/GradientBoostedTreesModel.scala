...
class GradientBoostedTreesModel(val model_Id: Int,
                        val logic: Map[Double, Double]) 
  extends GenericModelWithPredict2(model_Id, M_KIND.GBT){
  ...
  def numTrees(): Int = {
    ...
    val res: Int = JNISupport.gbtNumTrees(fs.master_node, this.mid)
    return res
  }
  def totalNumNodes(): Int = {
    val fs = FrovedisServer.getServerInstance()
    val res: Int = JNISupport.gbtTotalNumNodes(fs.master_node, this.mid)
    return res
  }
  def treeWeights(): Array[Double] = {
    val fs = FrovedisServer.getServerInstance()
    val res: Array[Double] = JNISupport.gbtTreeWeights(fs.master_node, this.mid)
    return res
  }
  override def toString(): String = {
    val fs = FrovedisServer.getServerInstance()
    val res: String = JNISupport.gbtToString(fs.master_node, this.mid)
    return res
  }
}
...
object GradientBoostedTreesModel {
  ...
  def load(sc: SparkContext, path: String): GradientBoostedTreesModel = {
    ...
    return new GradientBoostedTreesModel(modelId, logic)
  }
}
