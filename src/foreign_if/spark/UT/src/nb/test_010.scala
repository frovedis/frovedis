package test.scala;

import com.nec.frovedis.Jexrpc.FrovedisServer
import com.nec.frovedis.mllib.classification.{NaiveBayes, NaiveBayesModel}
import org.apache.spark.{SparkConf, SparkContext}
import org.apache.spark.mllib.util.MLUtils
import java.io.FileReader
import java.io.FileNotFoundException
import sys.process._

import java.io.IOException

// OBJECTIVE: To test Frovedis scala wrapper of NaiveBayes for load API

object GenericTest {

  def main(args: Array[String]): Unit = {

    // -------- configurations --------
    val conf = new SparkConf().setAppName("GenericTest").setMaster("local[2]")
    val sc = new SparkContext(conf)

    // initializing Frovedis server with "personalized command", if provided in command line
    if(args.length != 0) FrovedisServer.initialize(args(0))

    // -------- data loading from sample libSVM file at Spark side--------
    var data = MLUtils.loadLibSVMFile(sc, "./input/nb/bernoulli.txt")

    // -------- training --------
    val m1 = new NaiveBayes(0.8).setModelType("bernoulli").run(data)
    val m2 = new org.apache.spark.mllib.classification.NaiveBayes(0.8)
                .setModelType("bernoulli").run(data)
 
    // -------- model loading/saving --------
    // deleting the previous model file, if any
    var os_stat = "rm -rf ./out/NaiveBayesModel" .! 
    m1.save(sc, "./out/NaiveBayesModel")
    val loaded = NaiveBayesModel.load(sc, "./out/NaiveBayesModel")
    
    // -------- checking loaded model prediction accuracy --------
    var predictionAndLabel = data.map(p => (loaded.predict(p.features), p.label))
    val accuracy1 = 1.0 * predictionAndLabel.filter(x => x._1 == x._2).count() / data.count()

    predictionAndLabel = data.map(p => (m2.predict(p.features), p.label))
    val accuracy2 = 1.0 * predictionAndLabel.filter(x => x._1 == x._2).count() / data.count()

    if (accuracy1 == accuracy2) println("Status: Passed")
    else println("Status: Failed")
    
    // -------- clean-up --------
    os_stat = "rm -rf ./out/NaiveBayesModel" .! 
    m1.release()
    loaded.release() 
    FrovedisServer.shut_down()
    sc.stop()
  }
}

