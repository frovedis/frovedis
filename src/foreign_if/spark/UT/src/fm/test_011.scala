...
// Objective : test algo when  optimizer not correct  with Classification algo, isRegression : false
...
object GenericTest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      val model = FactorizationMachine.train(data,initStdev = 0.1,iter = 100, stepSize = 0.1,optimizer="DGD",
                                             isRegression = false,
                                             fitIntercept = true ,
                                             fitLinear = true,
                                             factorSize = 2,
