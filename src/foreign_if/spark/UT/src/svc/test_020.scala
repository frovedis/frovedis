...
object GenericTest {
  def main(args: Array[String]): Unit = {
    ...
    val model = new SVC().setC(C).setKernelType(kernelType).setDegree(degree).setGamma(gamma).setCoef0(coef0).setTol(tol).setCacheSize(cacheSize).setMaxIter(maxIter)

    if ((model.getC() != C) || (model.getKernelType() != kernelType) || (model.getDegree() != degree) || (model.getGamma() != gamma) || (model.getCoef0() != coef0) || (model.getTol() != tol) || (model.getCacheSize() != cacheSize) || (model.getMaxIter() != maxIter))
    {
