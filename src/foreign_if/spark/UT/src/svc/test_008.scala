...
object GenericTest {
  def main(args: Array[String]): Unit = {
    ...
    val model = new SVC().setC(C).setKernelType(kernelType).setDegree(degree).setGamma(gamma).setCoef0(coef0).setTol(tol).setCacheSize(cacheSize).run(d_lp)

    // --- prediction Vector---
    var test_data = sc.parallelize(data)
    val pred1 = model.predict(test_data).collect()
    // --- prediction save/load ---
    model.save(sc, "out/SVCModel_test_008")
    val loadedModel = SVCModel.load("out/SVCModel_test_008")
