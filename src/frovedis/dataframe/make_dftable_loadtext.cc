...
vector<size_t> extract_nulls(std::vector<int>& chars,
  ...
  vector<vector<size_t>> multi_nulls(nullstr_size);
  for(size_t i = 0; i < nullstr.size(); i++) {
    multi_nulls[i] = like(chars, starts, lens, nullstr[i]);
  }
  vector<size_t> nulls;
  set_multimerge(multi_nulls, nulls);
