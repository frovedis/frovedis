...
// ----- initcap -----
struct dffunction_initcap : public dffunction {
  dffunction_initcap(const std::shared_ptr<dffunction>& left) : left(left) {
    as_name = "initcap(" + left->get_as() + ")";
  }
  dffunction_initcap(const std::shared_ptr<dffunction>& left,
                     const std::string& as_name) :
    left(left), as_name(as_name) {}
  virtual std::string get_as() {return as_name;}
  virtual std::shared_ptr<dffunction> as(const std::string& cname) {
    as_name = cname;
    return std::make_shared<dffunction_initcap>(*this);
  }
  virtual std::shared_ptr<dfcolumn> execute(dftable_base& t) const;
  virtual std::shared_ptr<dfcolumn> execute(dftable_base& t1,
                                            dftable_base& t2) const {
    throw std::runtime_error
      ("initcap(): is not available for binary operation!\n");
  }
  virtual std::vector<std::shared_ptr<dfcolumn>>
  columns_to_use(dftable_base& t) {
    return left->columns_to_use(t);
...
std::shared_ptr<dffunction>
initcap_col(const std::string& left);

std::shared_ptr<dffunction>
initcap_col(const std::shared_ptr<dffunction>& left);

std::shared_ptr<dffunction>
initcap_col_as(const std::string& left, const std::string& as);

std::shared_ptr<dffunction>
initcap_col_as(const std::shared_ptr<dffunction>& left,
               const std::string& as);
...
struct dffunction_replace : public dffunction {
  dffunction_replace(const std::shared_ptr<dffunction>& left,
                     const std::string& from, const std::string& to) :
    left(left), from(from), to(to) {
    as_name = "replace(" + left->get_as() + "," + from + "," + to + ")";
  }
  dffunction_replace(const std::shared_ptr<dffunction>& left,
                     const std::string& from, const std::string& to,
                     const std::string& as_name) :
  ...
  virtual std::shared_ptr<dffunction> as(const std::string& cname) {
    as_name = cname;
    return std::make_shared<dffunction_replace>(*this);
  }
  virtual std::shared_ptr<dfcolumn> execute(dftable_base& t) const;
  virtual std::shared_ptr<dfcolumn> execute(dftable_base& t1, 
                                            dftable_base& t2) const {
    throw std::runtime_error
      ("replace is not available for binary operation!\n");
  }
  virtual std::vector<std::shared_ptr<dfcolumn>>
  columns_to_use(dftable_base& t) {
    return left->columns_to_use(t);
...
// ----- reverse -----
struct dffunction_reverse : public dffunction {
  dffunction_reverse(const std::shared_ptr<dffunction>& left) :
    left(left) {
    as_name = "reverse(" + left->get_as() + ")";
  }
  dffunction_reverse(const std::shared_ptr<dffunction>& left,
                     const std::string& as_name) :
    left(left), as_name(as_name) {}
  ...
  virtual std::shared_ptr<dffunction> as(const std::string& cname) {
    as_name = cname;
    return std::make_shared<dffunction_reverse>(*this);
  }
  virtual std::shared_ptr<dfcolumn> execute(dftable_base& t) const;
  virtual std::shared_ptr<dfcolumn> execute(dftable_base& t1, 
                                            dftable_base& t2) const {
    throw std::runtime_error
      ("reverse is not available for binary operation!\n");
  }
  virtual std::vector<std::shared_ptr<dfcolumn>>
  columns_to_use(dftable_base& t) {
    return left->columns_to_use(t);
...
// ----- concat -----
struct dffunction_concat : public dffunction {
  dffunction_concat(const std::shared_ptr<dffunction>& left, 
                    const std::shared_ptr<dffunction>& right) :
    left(left), right(right) {
    as_name = "concat(" + left->get_as() + "," + right->get_as() + ")";
  }
  dffunction_concat(const std::shared_ptr<dffunction>& left, 
                    const std::shared_ptr<dffunction>& right, 
                    const std::string& as_name) :
  ...
  virtual std::shared_ptr<dffunction> as(const std::string& cname) {
    as_name = cname;
    return std::make_shared<dffunction_concat>(*this);
  }
  virtual std::shared_ptr<dfcolumn> execute(dftable_base& t) const;
  virtual std::shared_ptr<dfcolumn> execute(dftable_base& t1,
                                            dftable_base& t2) const;
...
std::shared_ptr<dffunction>
concat_col(const std::string& left, const std::string& right); 
...
std::shared_ptr<dffunction>
concat_col_as(const std::string& left, const std::string& right,
              const std::string& as);
...
struct dffunction_concat_im : public dffunction {
  dffunction_concat_im(const std::shared_ptr<dffunction>& left,
                       const std::string& str,
                       bool is_reversed = false) :
    left(left), str(str), is_reversed(is_reversed) {
    if(is_reversed)
      as_name = "concat(" + str + "," + left->get_as() + ")";
    else
      as_name = "concat(" + left->get_as() + "," + str + ")";
  }
  dffunction_concat_im(const std::shared_ptr<dffunction>& left,
                       const std::string& str,
                       const std::string& as_name, bool is_reversed = false) :
    left(left), str(str), is_reversed(is_reversed), as_name(as_name) {}
  virtual std::string get_as() {return as_name;}
  virtual std::shared_ptr<dffunction> as(const std::string& cname) {
    as_name = cname;
    return std::make_shared<dffunction_concat_im>(*this);
  }
  virtual std::shared_ptr<dfcolumn> execute(dftable_base& t) const;
  virtual std::shared_ptr<dfcolumn> execute(dftable_base& t1,
                                            dftable_base& t2) const {
    throw std::runtime_error
      ("concat_im(): is not available for binary operation!\n");
  }
  virtual std::vector<std::shared_ptr<dfcolumn>>
  columns_to_use(dftable_base& t) {
    return left->columns_to_use(t);
...
std::shared_ptr<dffunction>
concat_multi_ws(const std::string& sep, 
                const std::vector<std::shared_ptr<dffunction>>& left);

std::shared_ptr<dffunction>
concat_multi_ws_as(const std::string& sep,
                   const std::vector<std::shared_ptr<dffunction>>& left,
                   const std::string& as);
...
struct dffunction_datetime_format_im : public dffunction {
  dffunction_datetime_format_im(const std::shared_ptr<dffunction>& left,
    ...
    left(left), fmt(fmt), type(type) {
    as_name = "datetime_format(" + left->get_as() + "," + fmt + ")";
  }
  dffunction_datetime_format_im(const std::shared_ptr<dffunction>& left,
                                const std::string& fmt,
                                const std::string& type,
                                const std::string& as_name) : 
    left(left), fmt(fmt), type(type), as_name(as_name) {}
  virtual std::string get_as() {return as_name;}
  virtual std::shared_ptr<dffunction> as(const std::string& cname) {
    as_name = cname;
    return std::make_shared<dffunction_datetime_format_im>(*this);
  }
  virtual std::shared_ptr<dfcolumn> execute(dftable_base& t) const;
  virtual std::shared_ptr<dfcolumn> execute(dftable_base& t1,
                                            dftable_base& t2) const {
    throw std::runtime_error
      ("datetime_format_im(): is not available for binary operation!\n");
  }
  virtual std::vector<std::shared_ptr<dfcolumn>>
  columns_to_use(dftable_base& t) {
    return left->columns_to_use(t);
