...
private:
  ...
  node_local<crs_matrix_local<int>> calc_graph(rowmajor_matrix<T>& vector_set,
    ...
    auto is_closer_than_eps = +[](rowmajor_matrix_local<T>& m, double& eps) {
      auto ret = crs_matrix_local<int>(m.local_num_row, m.local_num_col);
      auto* mvalp = m.val.data();

      size_t total_count = 0;
      for(size_t i=0;i<m.val.size();i++) {
        auto val = mvalp[i];
        total_count += (val < eps && val != 0);
      }
      ret.val.resize(total_count);
      ret.idx.resize(total_count);
      ret.off.resize(m.local_num_row + 1);
      ...
      for(size_t r=0;r<m.local_num_row;r++) {
        for(size_t c=0;c<m.local_num_col;c++){
          auto val = mvalp[r*m.local_num_col + c];
          if (val < eps && val != 0) {
            valp[current] = 1;
            idxp[current] = c;
            current++;
    ...
    // calc graph
    auto eps_l = make_node_local_broadcast(eps);
    return dist_m.data.template map(is_closer_than_eps, eps_l);
  };
