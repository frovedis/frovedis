...
template <typename T>
gmm_cluster<T> gmm_assign_cluster(frovedis::rowmajor_matrix_local<T> &NewData,
                                  int K, const std::string &path) {
  frovedis::colmajor_matrix_local<T> Data(NewData);
  auto input_mu = make_rowmajor_matrix_local_load<T>(path + "/model");
  auto input_cov = make_rowmajor_matrix_local_load<T>(path + "/model_cov");
  auto input_pi = make_rowmajor_matrix_local_load<T>(path + "/model_pi");
