#ifndef __RIDGE_REG__
#define __RIDGE_REG__
...
template <class T>
struct ridge_regression {
  ridge_regression(int max_iter = 1000,
                   double alpha = 0.01,
  ...
  set_params(std::vector<std::pair<std::string, param_t>>& data) {
    ...
    for (auto& e: data) {
      ...
      else REPORT_ERROR(USER_ERROR, "[ridge_regression] Unknown parameter: '" 
  ...
  _fit(MATRIX& mat, dvector<T>& label, 
    ...
    size_t n_iter = 0;
    if (solver == "sgd") {
      model = ridge_regression_with_sgd::train(
                      mat, label, model, sample_weight, 
                      n_iter, max_iter, alpha, mbf,
                      reg_param, fit_intercept, tol,
                      mat_type, input_movable);
    }
    else if (solver == "lbfgs") {
      model = ridge_regression_with_lbfgs::train(
                      mat, label, model, sample_weight,
                      n_iter, max_iter, alpha,
                      hist_size, reg_param, fit_intercept, tol,
                      mat_type, input_movable);
    }
  ...
  float score(MATRIX& mat, dvector<T>& label) {
    if(!is_fitted) REPORT_ERROR(USER_ERROR, 
                   "[ridge_regression] score is called before fit\n");
 ...
 ridge_regression& loadbinary(const std::string &inputPath) {
  ...
  ridge_regression& load(const std::string &inputPath) {
