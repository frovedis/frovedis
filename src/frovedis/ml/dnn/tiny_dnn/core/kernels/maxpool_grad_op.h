...
#ifdef __ve__
#include "tiny_dnn/core/kernels/maxpool_op_ve.h"
#endif
...
#ifdef __ve__
    } else if (engine == core::backend_t::ve) {
      const tensor_t &prev_data = context.input(0);  
      tensor_t &curr_data       = context.output(0); 
      kernels::maxpool_grad_op_ve(prev_data, prev_delta, curr_data, curr_delta, params.out2inmax,
                                  params.in2out, context.parallelize(), params);      
#endif
    } else {
