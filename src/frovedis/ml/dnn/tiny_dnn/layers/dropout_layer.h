...
class dropout_layer : public layer {
 public:
...
#else
    mask_.resize(1, std::vector<int>(in_dim));
#endif
    clear_mask();
  }
  ...
  void forward_propagation(const std::vector<tensor_t *> &in_data,
    ...
    size_t sz = in[0].size();
    std::vector<float> aslrand(sz * sample_count);
    asl_random_generate_s(get_asl_rng(), sz * sample_count, aslrand.data());
    for (size_t sample = 0; sample < sample_count; ++sample) {
      std::vector<int> &mask = mask_[sample];
      const vec_t &in_vec = in[sample];
      vec_t &out_vec      = out[sample];
     if (phase_ == net_phase::train) {
        for (size_t i = 0; i < sz; i++) {
          if (aslrand[sz * sample + i] < dropout_rate_)
            mask[i] = true;
          else
            mask[i] = false;
        }
...
#else
  const std::vector<int> &get_mask(size_t sample_index) const {
    return mask_[sample_index];
  }
#endif
