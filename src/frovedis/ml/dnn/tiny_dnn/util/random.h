...
#ifdef __ve__
inline asl_random_t& get_asl_rng() {
  static bool asl_is_initialized = false;
  static asl_random_t rng;
  if (asl_is_initialized == false) {
    asl_library_initialize();
    asl_random_create(&rng, ASL_RANDOMMETHOD_MT19937);
    asl_is_initialized = true;
  }
  return rng;
}
#endif
...
#else
  std::vector<float> aslrand(end - begin);
  asl_random_generate_s(get_asl_rng(), end - begin, aslrand.data());
  for (Iter it = begin; it != end; ++it)
	*it = min + aslrand[it - begin] * (max - min);
#endif
}
