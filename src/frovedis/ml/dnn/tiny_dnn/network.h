...
class network {
 public:
  ...
  vec_t fprop(const vec_t &in) {
    ...
    //a[0].emplace_back(in);//this is original
    a[0].resize(1, in); // resize is faster
    return fprop(a)[0][0];
#endif
  }
 ...
 private:
  ...
  void normalize_tensor(const std::vector<vec_t> &inputs,
                        std::vector<tensor_t> &normalized) {
    //normalized.reserve(inputs.size());//this is original
    normalized.resize(inputs.size());
    for (size_t i = 0; i < inputs.size(); i++) {
      //normalized.emplace_back(tensor_t{inputs[i]});//this is original
      //normalized.push_back(tensor_t{inputs[i]});
      size_t size = inputs[i].size();
      normalized[i].resize(1);
      normalized[i][0].resize(inputs[i].size());
      for (size_t j = 0; j < size; ++j)
        normalized[i][0][j] = inputs[i][j];
    }
  }
