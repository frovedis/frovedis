#ifndef _TSNE_IMPL_HPP_
#define _TSNE_IMPL_HPP_
...
T compute_grad_error(rowmajor_matrix<T>& P_mat,
  ...
  rowmajor_matrix<T> error_mat(P_mat.data.map(compute_error<T>, 
                               Q_mat.data,
                               broadcast(inv_sumq)));
  return matrix_sum(error_mat)[0];
}

template <class T>
double calc_matrix_norm(rowmajor_matrix<T>& mat) {
  return std::sqrt(matrix_squared_sum(mat)[0]);
}

template <class T>
void check_non_negative_mat(rowmajor_matrix<T>& mat) {
  auto res = mat.data.map(+[](rowmajor_matrix_local<T>& mat) -> int {
    return vector_count_negatives(mat.val) > 0;
  }).reduce(add<int>);
  require (res == 0, 
  "invalid matrix: matrix should contain positive elements!!\n");
}
