...
struct calc_dense_eigen_sym {
  ...
  calc_dense_eigen_sym(const std::string& order, int k, size_t mat_size,
                       T sigma = 0.0, int mode = 1) :
                       order(order), k(k), mat_size(mat_size),
                       sigma(sigma), mode(mode) {}
  void operator()(DENSE_MATRIX_LOCAL& mat,
                  diag_matrix_local<T>& d,
                  colmajor_matrix_local<T>& v) {
    dense_eigen_sym_mpi<T, DENSE_MATRIX_LOCAL>(mat,mat_size,d,v,order,k,sigma,mode);
  }
  std::string order;
  int k;
  size_t mat_size;
  T sigma;
  int mode;
  SERIALIZE(order, k, mat_size, sigma, mode)
};
...
void dense_eigen_sym(rowmajor_matrix<T>& mat,
  ...
  if(mat.num_col != mat.num_row) {
    throw std::runtime_error("dense_eigen_sym: matrix is not square");
  } else {
    ...
    new_mat.data.mapv(calc_dense_eigen_sym<T, DENSE_MATRIX_LOCAL>
                     (order,k,mat.num_row), dtmp, v.data);
  }
...
void dense_eigen_sym(rowmajor_matrix<T>& mat,
  ...
  if(mat.num_col != mat.num_row) {
    ...
    t.show("convert matrix format: ");
    new_mat.data.mapv(calc_dense_eigen_sym<T, DENSE_MATRIX_LOCAL>
                     (order,k,mat.num_row,sigma,3), // mode:3 is for shift-invert
                     dtmp, v.data);
  }
  d = *dtmp.get_dvid().get_selfdata();
  v.set_num(mat.num_row, k);
